...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  // Fetch JWT from local storage for API calls
              ...
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map((opt) => (
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-sm hover:bg-purple-50 flex items-center gap-2 transition-colors duration-200"
                  >
                    {opt.icon}
                    {opt.label}
                  </button>
